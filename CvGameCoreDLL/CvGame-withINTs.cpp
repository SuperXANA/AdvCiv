	...
	for (PlayerIter<CIV_ALIVE> itPlayer; itPlayer.hasNext(); ++itPlayer)
	{
			...
			itPlot.hasNext(); ++itPlot)
		{
				...
				MapRandSuccess(scaled(1, 1 + iDistance - iCityRange))))
			{
				...
				iPlotFood += GC.getCivilizationInfo(itPlayer->getCivilizationType()).getTerrainYieldChanges(p.getTerrainType(), YIELD_FOOD, p.isRiver());
				iPlotProduction += GC.getCivilizationInfo(itPlayer->getCivilizationType()).getTerrainYieldChanges(p.getTerrainType(), YIELD_PRODUCTION, p.isRiver());
				// XANA: 03-15-2025 FfH Civilization Terrain Yield Changes for AdvancedCiv
				...
				FOR_EACH_ENUM(Terrain)
				{
					...
					if (kRepl.isWater())
						continue;
					// XANA: 03-15-2025 FfH Civilization Terrain Yield Changes for AdvancedCiv
					if (kRepl.getYield(YIELD_FOOD) +
						GC.getCivilizationInfo(itPlayer->getCivilizationType()).getTerrainYieldChanges((int)eLoopTerrain, YIELD_FOOD, p.isRiver()) == iTargetFood && // advc.108: was >=
						kRepl.getYield(YIELD_FOOD) +
						kRepl.getYield(YIELD_PRODUCTION) +
						GC.getCivilizationInfo(itPlayer->getCivilizationType()).getTerrainYieldChanges((int)eLoopTerrain, YIELD_FOOD, p.isRiver()) +
						GC.getCivilizationInfo(itPlayer->getCivilizationType()).getTerrainYieldChanges((int)eLoopTerrain, YIELD_PRODUCTION, p.isRiver()) == iTargetTotal)
					{
						if (!p.isFeature() ||
			...
			iGoodPlot < 4; ++itPlot)
		{
				...
				GC.getFOOD_CONSUMPTION_PER_POPULATION())
			{
				FOR_EACH_ENUM(Terrain)
				{
					...
					GC.getCivilizationInfo(kPlayer.getCivilizationType()).getTerrainYieldChanges((int)eLoopTerrain, YIELD_FOOD, kPlot.isRiver()) >=
						GC.getFOOD_CONSUMPTION_PER_POPULATION())
					{
