	...
	bool bUnavailable) // advco.defr
{
	...
	// <advco.defr> (Tbd. - Make the unavailable color configurable)
		...
		szColTag = "COLOR_LIGHT_GREY"; // </advco.defr>
	{
		CvWString szTempBuffer;
	...
	CvPlot const& kPlot, bool bIndicator, bool bShort)
{
	...
	bool abAvailableDefender[iRogueIndex + 1] = {};
	// </advco.defr>
	FOR_EACH_UNIT_IN(pUnit, kPlot)
	{
		...
		if (!unavailable.empty() && pUnit->isEnemy(eActiveTeam))
			abAvailableDefender[iOwnerIndex] = true; // </advco.defr>
		int iType = OTHER;
	...
	/*	advco.defr: (OK to be unavailable while resolving a failed attack
		against kCenterUnit. Seems that the info bar is then always dirty.) */
	FAssert(kCenterUnit.isInfoBarDirty() || unavailable.count(&kCenterUnit) <= 0);
	...
	do
	{
		...
		m_pHelpPlot = &kPlot;
		// advco.defr:
		m_pUnavailable = (abAvailableDefender[iNextIndex] ? &unavailable : NULL);
		if(bAggregate)
		{
	...
	bool bIndicator) // advc.061, advc.007
{
	...
	if (iNumVisibleUnits > 0)
	{
		...
		std::set<CvUnit const*> unavailable;
		kPlot.unavailableDefendersVsActivePlayer(unavailable); // </advco.defr>
