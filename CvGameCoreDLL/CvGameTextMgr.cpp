	...
	CivilizationTypes eCivilization, bool bDawnOfMan)
{
	...
	// XANA: 03-22-2025 FfH Unit Upgrade Cost Modifiers for AdvancedCiv
	{
		int iLast = 0;
		FOR_EACH_ENUM(UnitCombat)
		{
			int iUpgradeCostModifier = kTrait.getUpgradeCostUnitCombatModifier(eLoopUnitCombat);
			if (iUpgradeCostModifier != 0)
			{
				szText = gDLL->getText("TXT_KEY_UPGRADE_COST_MODIFIER_HELP",
						iUpgradeCostModifier);
			}			
				CvWString szUnitCombat;
				szUnitCombat.Format(L"<link=literal>%s</link>", GC.getInfo(eLoopUnitCombat).getTextKeyWide());
				setListHelp(szHelpString, szText.GetCString(),
					szUnitCombat, L", ",
					iLast, iUpgradeCostModifier);
		}
	}
}	// XANA: 03-22-2025 FfH Unit Upgrade Cost Modifiers for AdvancedCiv
