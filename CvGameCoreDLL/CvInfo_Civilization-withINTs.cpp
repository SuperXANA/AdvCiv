...
CvCivilizationInfo::~CvCivilizationInfo()
{
	...
	if (m_ppiTerrainYieldChanges != NULL)
	{
		...
		{
			SAFE_DELETE_ARRAY(m_ppiTerrainYieldChanges[(int)eLoopTerrain]);
		}
	...
	if (m_ppiTerrainRiverYieldChanges != NULL)
	{
		FOR_EACH_ENUM(Terrain)
		{
			SAFE_DELETE_ARRAY(m_ppiTerrainRiverYieldChanges[(int)eLoopTerrain]);
		}
...
void CvCivilizationInfo::setTerrainYieldChanges(int i, int j, int k, bool is_river) const
{
	FAssertMsg(i < GC.getNumTerrainInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	FAssertMsg(j < NUM_YIELD_TYPES, "Index out of bounds");
	...
	if (is_river)
	{
		m_ppiTerrainRiverYieldChanges[i][j] = k;
	}
	if (!is_river)
	{
