				...
				MapRandSuccess(scaled(1, 1 + iDistance - iCityRange))))
			{
				...
				FOR_EACH_ENUM(Terrain)
				{
					...
					if (kRepl.getYield(YIELD_FOOD) +
						GC.getCivilizationInfo(itPlayer->getCivilizationType()).getTerrainYieldChanges(eLoopTerrain, YIELD_FOOD, p.isRiver()) == iTargetFood && // advc.108: was >=
						kRepl.getYield(YIELD_FOOD) +
						kRepl.getYield(YIELD_PRODUCTION) +
						GC.getCivilizationInfo(itPlayer->getCivilizationType()).getTerrainYieldChanges(eLoopTerrain, YIELD_FOOD, p.isRiver()) +
						GC.getCivilizationInfo(itPlayer->getCivilizationType()).getTerrainYieldChanges(eLoopTerrain, YIELD_PRODUCTION, p.isRiver()) == iTargetTotal)
					{
						if (!p.isFeature() ||
