	...
	bool bObsolete) const // advc.004c
{
	...
	for (int iPass = 0; iPass < 2; iPass++)
	{
		...
		if (iFocusFlags & BUILDINGFOCUS_EXPERIENCE || iPass > 0)
		{
			...
			{
				// XANA: 06-07-2025 Leader-Specific Favorite Unit Combat Type
				int iFavoriteUnitCombatWeight = kPersonality.getFavoriteUnitCombatWeight(GC.getInfo(perUnitCombatVal.first).getUnitCombatType());
				// XANA: 06-07-2025 Leader-Specific Favorite Unit Combat Type
				if (canTrain(perUnitCombatVal.first))
	// XANA: 06-07-2025 Leader-Specific Favorite Unit Combat Type
					iValue += (perUnitCombatVal.second * (iWeight + (iFavoriteUnitCombatWeight > 0 ? iFavoriteUnitCombatWeight : 0))
	// XANA: 06-07-2025 Leader-Specific Favorite Unit Combat Type
				) / 2;
			}
			...
			for (int i = 0; i < kCiv.getNumUnits(); i++)
			{
				...
				if(canTrain(eUnit))
				{
					iValue += (kBuilding.getUnitCombatFreeExperience(eCombatType) *
							(iHasMetCount > 0 ? 6 : 3));
	// XANA: 06-07-2025 Leader-Specific Favorite Unit Combat Type
					int iFavoriteUnitCombatWeight = kPersonality.getFavoriteUnitCombatWeight(eCombatType);
					if (iFavoriteUnitCombatWeight > 0)
						iValue += (5 * iFavoriteUnitCombatWeight)
	// XANA: 06-07-2025 Leader-Specific Favorite Unit Combat Type
				}
			}
