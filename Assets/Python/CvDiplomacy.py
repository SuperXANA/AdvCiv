...
class CvDiplomacy:
	...
	def filterUserResponse(self, diploInfo):
		...
		for i in range(diploInfo.getNumResponses()):	
			...
			# check civ type - OTHER CIVILIZATION
			if (self.isUsed(diploInfo.getOtherCivilizationTypes, i, gc.getNumCivilizationInfos()) and
				not diploInfo.getOtherCivilizationTypes(i, ourPlayer.getCivilizationType())):
				continue
				
			# check civ type - OTHER CIVILIZATION NEGATED
			if (self.isUsed(diploInfo.getOtherNotCivilizationTypes, i, gc.getNumCivilizationInfos()) and
				diploInfo.getOtherNotCivilizationTypes(i, ourPlayer.getCivilizationType())):
				continue
			# XANA: 03-29-2025 Leader_Amd_Civilization-Specific Diplomacy Texts for AdvancedCiv
			...
			# XANA: 03-29-2025 Leader_Amd_Civilization-Specific Diplomacy Texts for AdvancedCiv			
			# check leader type - OTHER LEADER
			if (self.isUsed(diploInfo.getOtherLeaderHeadTypes, i, gc.getNumLeaderHeadInfos()) and
				not diploInfo.getOtherLeaderHeadTypes(i, ourPlayer.getLeaderType())):
				continue
				
			# check leader type - OTHER LEADER NEGATED
			if (self.isUsed(diploInfo.getOtherNotLeaderHeadTypes, i, gc.getNumLeaderHeadInfos()) and
				diploInfo.getOtherNotLeaderHeadTypes(i, ourPlayer.getLeaderType())):
				continue
			# XANA: 03-29-2025 Leader_Amd_Civilization-Specific Diplomacy Texts for AdvancedCiv
